...
fn main() -> Result<(), Error> {
    ...
    let source_row = clap::value_t!(matches.value_of("source_row"), u32)?;
    let source_column = clap::value_t!(matches.value_of("source_column"), u32)?;
    let target_height = clap::value_t!(matches.value_of("target_height"), i32)?;
    let target_width = clap::value_t!(matches.value_of("target_width"), i32)?;
    let zoom_factor = {
        let val = clap::value_t!(matches.value_of("zoom_factor"), f32)?;
        if val < 1.0 {
