...
fn basic_usage(filename: &str) -> Result<(), Error> {
    ...
    let levels = unsafe { bindings::get_level_count(osr)? };
    println!("Slide has {} levels", levels);

    let (height, width) = unsafe { bindings::get_level0_dimensions(osr)? };
    println!("Slide has dimension {} x {} at level 0", height, width);

    let level = 0;
    let (height, width) = unsafe { bindings::get_level_dimensions(osr, level)? };
    println!(
        "Slide has dimension {} x {} at level {}",
        height, width, level
    );

    let factor = unsafe { bindings::get_level_downsample(osr, level)? };
    println!(
    ...
    let level = unsafe { bindings::get_best_level_for_downsample(osr, downsample_factor)? };
    println!(
    ...
    let buffer = unsafe { bindings::read_region(osr, x, y, level, w, h)? };
